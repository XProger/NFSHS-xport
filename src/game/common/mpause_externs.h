...
extern "C" extern int   gMasterMusicLevel, gMasterSFXLevel, gMasterEngineLevel,
...
extern "C" { extern void  AudioMus_Volume(int level);                            /* audiomus.obj */ }
extern int   AudioCmn_MusicLevel(int masterLevel);                  /* audiocmn.obj */
extern "C" { extern int   AudioCmn_PlaySound(int bnkID, int a, int b, int c, int d);   /* audiocmn.obj */ }

/* ---- cross-TU functions (input / system) ---- */
extern int Debounce(void *menu);
extern "C" { extern int   Input_Interface(int key, int debounce); }
extern "C" { extern void  InGame_ResetPSXController(u_int port, int config); }
                                    /* libgpu */
extern void  TextSys_LoadInGame(int language);                      /* textsys.obj */
extern "C" { extern void  TextSys_UnloadWords(void);                             /* textsys.obj */ }
