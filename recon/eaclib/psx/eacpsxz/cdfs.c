...
extern "C" int addsystemtask(intptr_t fn, int period, int delay); /* @ systask */
extern "C" intptr_t delsystemtask(intptr_t fn);                    /* @ systask */
...
extern "C" void CD_systaskfunc(void)
{
    ...
    if (done) {
        delsystemtask((intptr_t)CD_systaskfunc);   /* remove ourselves */
        CdReset(0);
        mode[0] = 0xA0;
...
extern "C" void CD_timerfunc(void)
{
    if (CD_timeout != 0) {
        ...
        if (CD_timeout == 0) {
            addsystemtask((intptr_t)CD_systaskfunc, 0, 0);
            CD_timeout = 0;
            deltimer((void *)CD_timerfunc);
        }
    }
