...
#if defined(AP_WIN)
extern "C" {
void PsyQSpuKeyOn(unsigned int);
void PsyQSpuKeyOff(unsigned int);
}
#endif
...
extern "C" void iSNDcalcvol(int chan)
{
    ...
    if (v->f50 != 0) {                              /* 32-bit PSX velocity-curve pointer word */
    ...
    if (v->f44 != 0) {                              /* 32-bit PSX final-curve pointer word */
...
#if defined(AP_WIN)
    PsyQSpuKeyOn((unsigned int)mask);
#endif
    return DAT_80147e2c;
}
...
#if defined(AP_WIN)
    PsyQSpuKeyOff((unsigned int)mask);
#endif
    return DAT_80147e2c;
}
